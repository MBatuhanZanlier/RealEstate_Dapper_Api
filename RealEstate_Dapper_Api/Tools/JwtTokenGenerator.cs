    ...
    { 
            ...
            public static TokenResposeViewModel GenerateToken(GetCheckAppUserViewModel model)
            {
                ...
                if (!string.IsNullOrWhiteSpace(model.UsurName))
                    claims.Add(new Claim("UserName", model.UsurName));
                ...
                JwtSecurityToken token = new JwtSecurityToken(issuer: JwTokenDefaults.ValidIssuer, audience: JwTokenDefaults.ValidAudience, claims: claims, notBefore: DateTime.UtcNow, expires: expireDate, signingCredentials: signinCredentials);
